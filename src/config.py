...
DEFAULTS = {
    ...
    # 土地の配点。戸建・マンションと主役が違う。建っているものではなく、
    # これから建てるものを計算するので「建てられる家」と「接道」が前に出る。
    # リスクが15→25なのは、土地では地盤改良が実費でそのまま乗ってくるため。
    # 価格は採点しない（㎡単価のばらつきが大きく、点にする精度が出ない）。
    "land_category_weights": {"建てられる家": 25, "接道": 15, "リスク": 25,
                              "立地": 15, "資産性": 10, "資金": 10},
